...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  const masterDataChildren = [
    {
      link: "/master-data/pasten",
      label: "Pasten",
    },
    {
      link: "/master-data/saluran",
      label: "Saluran",
    },
  ];
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        <Link href="/">
          <Image
            width={176}
            height={32}
            src={"/images/logo/logo_pupr_2_w.png"}
            alt="Logo"
          />
        </Link>
              ...
              <li>
                <Link
                  ...
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    pathname.includes("plant-pattern") &&
                    ...
                    <React.Fragment>
                      ...
                      >
                        <ul className="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
                          {masterDataChildren.map(
                            (masterData, indexMasterData) => (
                              <li key={`masterData${indexMasterData}`}>
                                <Link
                                  href={masterData.link}
                                  className={`first-letter:group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                    pathname === masterData.link && "text-white"
                                  }`}
                                >
                                  {masterData.label}
                                </Link>
