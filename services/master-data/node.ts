...
const getDatas = async (
  ...
  try {
    const axiosResponse = await axios.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/${process.env.NEXT_PUBLIC_API_VERSION}/nodes?limit=${limit}&page=${page}`
    );
...
const getDataId = async (id: string, callBack: (data: any) => void) => {
  try {
    const axiosResponse = await axios.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/${process.env.NEXT_PUBLIC_API_VERSION}/nodes/${id}`
    );
    const response: ApiResponse<any> = axiosResponse.data;
